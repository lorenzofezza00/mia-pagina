// src/components/ScrollMap.js
import React, { useEffect, useMemo, useState } from "react";
import { createPortal } from "react-dom";
import { MapContainer, TileLayer, Marker, Polyline, useMap } from "react-leaflet";
import L from "leaflet";
import "leaflet/dist/leaflet.css";
import { FiHome, FiMapPin, FiX, FiChevronLeft, FiChevronRight } from "react-icons/fi";
...
const galleriesData = {
  ...
  torino: { title: "Torino, PoliTO", photos: ["https://picsum.photos/seed/tor/800/600"] },
  laCoruna: { title: "La Coruña, Spagna", photos: ["https://picsum.photos/seed/cor1/800/600", "https://picsum.photos/seed/cor2/800/600"] },
  edinburgh: { title: "Edinburgh, Scotland", photos: ["https://picsum.photos/seed/edi/800/600"] },
  barcelona: { title: "Barcellona, Spagna", photos: ["https://picsum.photos/seed/bcn/800/600"] },
  valfrejus: { title: "Valfréjus, Francia", photos: ["https://picsum.photos/seed/val/800/600"] },
  bolzano: { title: "Bolzano, Trentino", photos: ["https://picsum.photos/seed/bol/800/600"] },
  milano: { title: "Milano, Lombardia", photos: ["https://picsum.photos/seed/mil/800/600"] },
  firenze: { title: "Firenze, Toscana", photos: ["https://picsum.photos/seed/fir/800/600"] },
  roma: { title: "Roma, Lazio", photos: ["https://picsum.photos/seed/rom/800/600", "https://picsum.photos/seed/rom2/800/600"] },
  matera: { title: "Matera, Basilicata", photos: ["https://picsum.photos/seed/mat/800/600"] },
  lecce: { title: "Lecce, Puglia", photos: ["https://picsum.photos/seed/lec/800/600"] }
};
...
export default function ScrollMap({ sentence3Ref }) {
  ...
  // FIX: NASCONDE LE SCRITTE QUANDO APRI LE FOTO
  const openGallery = (placeId) => {
    setActiveGallery(placeId);
    setCurrentImgIndex(0);
    // Blocca lo scroll
    document.body.style.overflow = "hidden";
    // Nasconde fisicamente il container delle scritte
    ...
    <>
      <style>
        {`
          ...
          /* SFONDO DELLA GALLERIA NERO TOTALE */
          .gallery-overlay {
            position: fixed;
            top: 0;
            left: 0;
      ...
      {/* OVERLAY TELETRASPORTATO CON PORTALS */}
      {activeGallery && galleriesData[activeGallery] && createPortal(
        <div className="gallery-overlay" onClick={closeGallery}>
          ...
          >
            {galleriesData[activeGallery].photos.length > 1 && !isMobile && (
              <div className="gallery-controls">
                <div className="gallery-btn" onClick={prevImg}><FiChevronLeft size={28} /></div>
                <div className="gallery-btn" onClick={nextImg}><FiChevronRight size={28} /></div>
              </div>
            )}

            <img 
              src={galleriesData[activeGallery].photos[currentImgIndex]} 
              alt={galleriesData[activeGallery].title} 
              className="gallery-img"
            />
          </div>
          
          <div style={{ position: "absolute", bottom: "3rem", fontSize: "1.2rem", letterSpacing: "2px", zIndex: 2147483647 }}>
            {currentImgIndex + 1} / {galleriesData[activeGallery].photos.length}
          </div>
          {isMobile && galleriesData[activeGallery].photos.length > 1 && (
            <div style={{ position: "absolute", bottom: "1.5rem", fontSize: "0.8rem", opacity: 0.6, zIndex: 2147483647 }}>
              Scorri per navigare
            </div>
          )}
        </div>,
